...
class ${document.querySelector("#infohead h1").textContent} extends ${[...document.querySelectorAll("div.superPaths a:not(:last-of-type)")].map(a=>a.textContent).slice(-1)[0]}{
...
${[...document.querySelectorAll(".definition-table:first-of-type tbody .supertype:first-of-type ~ tr:not(.supertype):not(.not-to-consider)")].map(row => {
    ...
    public function description():string {
        return __("${document.querySelector("div.description").textContent}", 'jsonld-for-wordpress');
    }
